...
7. 多设备使用时，在设置中配置 WebDAV。应用数据同步、本地漫画 CBZ 归档、WebDAV 漫画库在线阅读是三类独立配置。
...
- 本地漫画相关导入导出使用 CBZ/ZIP/7Z 等压缩包格式，便于备份、迁移和分发。
- 如果需要跨设备迁移本地漫画，可结合 WebDAV 本地漫画归档、恢复和删除功能使用。

### WebDAV 漫画库

- WebDAV 漫画库是一种在线阅读渠道，适合把漫画按目录和图片存放在 WebDAV 服务端后按需加载。
- 在线阅读只支持目录图片结构，例如 `漫画名/章节名/001.jpg`；不会把远端 CBZ/ZIP/7Z 当作在线预览内容。
- WebDAV 上的 CBZ 仍可用于归档、上传、下载和恢复，但它属于分发与备份格式，不属于在线阅读格式。
...
- 应用数据同步、本地漫画 CBZ 归档、WebDAV 漫画库在线阅读是三类能力：前者偏设置、收藏、历史等数据，归档偏本地漫画迁移，在线漫画库偏远端目录按需阅读。
